...
class TodosVMTest {
    ...
    fun checkCheckbox() {
        ...
        assertThat(todosStates.filterIsInstance<Loadable.Success<TodoModels>>().map {
            it.data.single().state
        })
    ...
    fun shouldPerformDelete() {
        ...
        // then
        coVerify(exactly = 1) { todoClient.delete(todoDto.id) }
        coVerify(exactly = 2) { todoClient.all() }
        ...
        assertThat(states[0]).hasSize(1) // first - initial state with single todo
